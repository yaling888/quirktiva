...
func TestClash_Hysteria2ObfsSalamander(t *testing.T) {
	cfg := &container.Config{
		Image:        ImageHysteria2,
		ExposedPorts: defaultExposedPorts,
		Entrypoint:   []string{"hysteria"},
	...
	hostCfg := &container.HostConfig{
		PortBindings: defaultPortBindings,
		Binds: []string{
			fmt.Sprintf("%s:/app/config.yaml", C.Path.Resolve("hysteria2-obfs-salamander.yaml")),
			fmt.Sprintf("%s:/app/fullchain.crt", C.Path.Resolve("example.org.pem")),
			fmt.Sprintf("%s:/app/privkey.key", C.Path.Resolve("example.org-key.pem")),
		},
	}

	id, err := startContainer(cfg, hostCfg, "hysteria2-obfs-salamander")
	require.NoError(t, err)
...
func TestClash_Hysteria2ObfsGecko(t *testing.T) {
	cfg := &container.Config{
		Image:        ImageHysteria2,
		ExposedPorts: defaultExposedPorts,
		Entrypoint:   []string{"hysteria"},
	...
	hostCfg := &container.HostConfig{
		PortBindings: defaultPortBindings,
		Binds: []string{
			fmt.Sprintf("%s:/app/config.yaml", C.Path.Resolve("hysteria2-obfs-gecko.yaml")),
			fmt.Sprintf("%s:/app/fullchain.crt", C.Path.Resolve("example.org.pem")),
			fmt.Sprintf("%s:/app/privkey.key", C.Path.Resolve("example.org-key.pem")),
		},
	}

	id, err := startContainer(cfg, hostCfg, "hysteria2-obfs-gecko")
	require.NoError(t, err)

...
func NewClient(ctx context.Context, dialOut util.DialOutFunc, _padding *atomic.Pointer[padding.PaddingFactory],
	idleSessionCheckInterval, idleSessionTimeout time.Duration, minIdleSession int,
) *Client {
	c := &Client{
...
func (c *Client) CreateStream(ctx context.Context, opts ...dialer.Option) (net.Conn, error) {
	select {
	case <-c.die.Done():
...
func (c *Client) createSession(ctx context.Context, opts ...dialer.Option) (*Session, error) {
	underlying, err := c.dialOut(ctx, opts...)
	if err != nil {
		return nil, err
	}

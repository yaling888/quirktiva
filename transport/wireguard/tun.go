...
type netTun struct {
	...
	dnsCache       *cache.LruCache[string, []netip.Addr]
	hasV4, hasV6   bool
}
...
func CreateNetTUN(localAddresses, dnsServers []netip.Addr, mtu int) (tun.Device, *Net, error) {
	...
	dev.dnsCache = cache.New[string, []netip.Addr](cache.WithSize[string, []netip.Addr](4096), cache.WithStale[string, []netip.Addr](true))
...
func (pc *PingConn) SetWriteDeadline(_ time.Time) error {
...
func (net *Net) LookupHost(host string) (addrs []netip.Addr, err error) {
...
func (tnet *Net) LookupContextHost(ctx context.Context, host string) ([]netip.Addr, error) {
	...
	addrsCache, expireTime, hit := tnet.dnsCache.GetWithExpire(host)
	if hit {
		now := time.Now()
		if now.Before(expireTime) {
			return addrsCache, nil
		}
	}
	...
	tnet.dnsCache.SetWithExpire(host, addrs, time.Now().Add(time.Second*time.Duration(ttl)))
	return addrs, nil
}
